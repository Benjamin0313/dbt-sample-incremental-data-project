...
| `generate.py --daemon --interval 60 --minutes 30` | 60秒ごとに**30分ぶん** | 常駐＋**早送り**(実時間の30倍速) |
...
**早送り**したいときは `--minutes` を併用。`--daemon` と一緒だと **1 tick ごとにその分数ぶん**生成する:

```bash
# 60秒ごとに「30分ぶん」を投入(実時間の30倍速でデータが溜まる)
uv run python generate.py --daemon --interval 60 --minutes 30
# → 毎 tick: customers +6 / orders +60
```
...
#### `new_per_min` は「平均レート」。実際の挿入タイミングは `--interval` で決まる

`new_per_min` は**平均ペース**にすぎない。デーモンは tick(=`--interval` 秒)ごとに
`new_per_min × (interval ÷ 60)` を `_datagen_state` の端数に積み増し、**1以上になった整数分だけ insert** する。
そのため `new_per_min: 2`（=平均で1分に2件）でも、実際の出方は `--interval` 次第:

| `--interval` | 1 tick の積み増し | 実際の挿入 |
| --- | --- | --- |
| `30` | `2 × 0.5 = 1.0` | **30秒ごとに1件**ずつ |
| `60` | `2 × 1.0 = 2.0` | **60秒ごとに2件**まとめて |
| `10` | `2 × 0.167 = 0.33` | 0,0,**1**,0,0,**1**… ≒ **約30秒ごとに1件** |
| 単発 `--minutes 30` | `2 × 30 = 60` | **一度に60件** |

どの `--interval` でも長い目で見た合計は同じ（平均が保たれる）。違うのは「細かく出るか／まとめて出るか」だけ。
「きっかり30秒ごとに1件ずつ」入れたいなら `--daemon --interval 30` で回す。
